 ...
 */
        ...
        template<typename M>
        void ensure_equals(const M& msg, const std::wstring& actual, const std::wstring& expected)
        {
            const std::string actualConverted = wm_cast<std::string>(actual);
            const std::string expectedConverted = wm_cast<std::string>(expected);
            ensure_equals(msg, actualConverted, expectedConverted);
        }

        template<typename M>
        void ensure_equals(const M& msg, const std::wstring& actual, const wchar_t* expected)
        {
            const std::string actualConverted = wm_cast<std::string>(actual);
            const std::string expectedConverted = wm_cast<std::string>(expected);
            ensure_equals(msg, actualConverted, expectedConverted);
        ...
        template<typename M>
        void ensure_equals(const M& msg, const wchar_t* actual, const wchar_t* expected)
        {
            const std::string actualConverted = wm_cast<std::string>(actual);
            const std::string expectedConverted = wm_cast<std::string>(expected);
            ensure_equals(msg, actualConverted, expectedConverted);
        ...
        template<typename M>
        void ensure_equals(const M& msg, const wchar_t* actual, const std::wstring& expected)
        {
            const std::string actualConverted = wm_cast<std::string>(actual);
            const std::string expectedConverted = wm_cast<std::string>(expected);
            ensure_equals(msg, actualConverted, expectedConverted);
 ...
 */
        ...
        template<typename M>
        void ensure_equals(const M& msg,
            const double& actual,
        ...
        {
            typename std::iterator_traits<LhsIterator>::difference_type lhs_size = std::distance(
                lhs_begin,
                lhs_end);
            typename std::iterator_traits<RhsIterator>::difference_type rhs_size = std::distance(
                rhs_begin,
                rhs_end);
