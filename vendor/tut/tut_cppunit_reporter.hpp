...
public:
        ...
        void run_completed()
        {
                ...
                for(unsigned int i = 0; i < failed_tests_.size(); i++){
                    ...
                    case test_result::ex_ctor:
                        ...
                        failure_msg = "Constructor has thrown an exception: "
                            + failed_tests_[i].exception_typeid + '\n';
                        errors++;
                        break;
                    ...
                    *stream_ << "    <FailedTest id=\"" << failed_tests_[i].test << "\">"
                             << std::endl
                             << "      <Name>" << encode(failed_tests_[i].group) + "::" + encode(
                        failed_tests_[i].name) << "</Name>" << std::endl
                             ...
                             << "      <Message>"
                             << encode(failure_msg
                        + failed_tests_[i].message) << "</Message>" << std::endl
                             << "    </FailedTest>" << std::endl;
                }
                ...
                *stream_ << "  <SuccessfulTests>" << std::endl;
                ...
                for(unsigned int i = 0; i < passed_tests_.size(); i++){
                    *stream_ << "    <Test id=\"" << passed_tests_[i].test << "\">" << std::endl
                             << "      <Name>" << encode(passed_tests_[i].group) + "::" + encode(
                        passed_tests_[i].name) << "</Name>" << std::endl
                             << "    </Test>" << std::endl;
                }
            ...
            *stream_ << "  <Statistics>" << std::endl
                     << "    <Tests>"
                     << (failed_tests_.size() + passed_tests_.size()) << "</Tests>" << std::endl
                     << "    <FailuresTotal>" << failed_tests_.size() << "</FailuresTotal>"
                     << std::endl
