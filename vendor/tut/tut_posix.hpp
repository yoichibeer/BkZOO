            ...
            if(!pids_.empty()){
                ...
                for(std::map<pid_t, int>::iterator i = pids_.begin(); i != pids_.end(); ++i){
                    try{
                        ...
                        ss << std::endl << "child " << ex.tr.pid << " has thrown an exception: "
                           << ex.what();
                    }
        ...
        void kill_child_(pid_t pid)
        {
            ...
            if(::kill(pid, SIGTERM) != 0){
                if(errno == ESRCH){
                    ...
                    std::ostringstream ss;
                    char e[1024];
                    ss << "child " << pid << " could not be killed with SIGTERM, " << strerror_r(
                        errno,
                        e,
                        sizeof(e)) << std::endl;
                    fail(ss.str());
            ...
            if(waitpid_(pid, &status, WNOHANG) != pid){
                ...
                if(::kill(pid, SIGKILL) != 0){
                    if(errno == ESRCH){
                        ...
                        std::ostringstream ss;
                        char e[1024];
                        ss << "child " << pid << " could not be killed with SIGKILL, "
