...
namespace tut
{
    namespace detail
    {
        ...
        struct fpt_traits
        {
            ...
            static const StdNumericLimitsNotAvailable static_check[std::numeric_limits < T
            > ::is_specialized];
        ...
        {
            typename fpt_traits<T>::Result diff
                = fpt_traits<T>::div(fpt_traits<T>::abs(expected - actual),
    ...
    template<typename T, typename U>
    void ensure_close_fraction(const char* msg,
        const T& actual,
        const T& expected,
        const U& fraction)
    {
        typedef char StdNumericLimitsNotAvailable;
        const StdNumericLimitsNotAvailable static_check[std::numeric_limits < U
        > ::is_specialized] = {0};
        static_cast<void>(static_check);
