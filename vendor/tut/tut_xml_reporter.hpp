 ...
 */
    class xml_reporter : public tut::callback
    {
         ...
         */
        std::string xml_build_testcase(const tut::test_result & tr,
            ...
            int pid = 0)
        {
            ...
            if((tr.result == test_result::ok)
                ...
                out << "    <testcase classname=\"" << cppunit_reporter::encode(tr.group)
                    << "\" name=\"" << cppunit_reporter::encode(tr.name) << "\"/>";
            }
            else{
                ...
                out << "    <testcase classname=\"" << cppunit_reporter::encode(tr.group)
                    << "\" name=\"" << cppunit_reporter::encode(tr.name) << "\">" << endl;
                out << "      <" << tag << " message=\"" << err_msg << "\"" << " type=\""
                    << failure_type << "\"";
            ...
            const std::string & name, const std::string & testcases)
        {
            ...
            out << "  <testsuite errors=\"" << errors << "\" failures=\"" << failures
                << "\" tests=\"" << total << "\" name=\"" << cppunit_reporter::encode(name)
                << "\">" << std::endl;
            out << testcases;
            out << "  </testsuite>";
...
public:
        ...
        {
            /* *********************** header ***************************** */
            *stream_ << "<?xml version=\"1.0\" encoding=\"utf-8\" standalone=\"yes\"?>"
                     << std::endl;
            *stream_ << "<testsuites>" << std::endl;

            // iterate over all test groups
            for(TestGroups::const_iterator tgi = all_tests_.begin(); tgi != all_tests_.end();
                ++tgi){
                ...
                for(TestResults::const_iterator tri = results.begin(); tri != results.end(); ++tri){
                    ...
                    case test_result::ex_ctor:
                        failure_type = "Assertion";
                        failure_msg = "Constructor has thrown an exception: "
                            + tri->exception_typeid + ".\n";
                        exceptions++;
                        break;
                    case test_result::rethrown:
                        failure_type = "Assertion";
...
#if defined(TUT_USE_POSIX)
                    out
                        << xml_build_testcase(*tri, failure_type, failure_msg,
                        tri->pid) << std::endl;
#else
                ...
                *stream_ << xml_build_testsuite(stat_errors,
                    stat_failures,
                    stat_all,
                    (*tgi).first /* name */,
                    out.str() /* testcases */) << std::endl;
            } // iterate over all test groups
